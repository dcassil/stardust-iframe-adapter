 ...
 * (no build artifacts required) while giving the adapter its real peers.
 */
...
export default defineConfig({
  ...
  test: {
    // Global jsdom so both the iframe- and host-side `.tsx` React tests run
    // without per-file docblocks. The framework-agnostic protocol/host unit
    // tests run fine under jsdom too.
    environment: "jsdom",
    globals: false,
